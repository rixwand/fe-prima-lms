...
export function PendingCourseCardGrid({
  data,
  onPublish,
  onUnpublish,
  onDelete,
...
export function PendingCourseCardList({
  data,
  onPublish,
  onUnpublish,
  onDelete,
  ...
  return (
    <Card
      ...
      onClick={prefetch}>
      {/* <Chip
        startContent={<LuClockAlert size={10} />}
        classNames={{
          content: "px-1 text-[10px] py-0",
        }}
        size="sm"
        className="absolute left-4 -top-3 shadow-sm text-white"
        // color={data.status == "DRAFT" ? "warning" : data.status == "PUBLISHED" ? "success" : "default"}
        color="warning"
        variant={"solid"}>
        PENDING
...
export function PendingCourseCardGridSkeleton() {
  return (
...
export function PendingCourseCardListSkeleton() {
  return (
