...
const MediaForm = ({
  ...
  defaultValues: { coverImage: string; previewVideo: string | null };
}) => {
  const {
    getValues,
    formState: { errors },
  ...
  const validUrlPreview = urlPreview ? getYouTubeEmbedUrl(urlPreview) : null;
    ...
    <div className="space-y-6 rounded-2xl border border-slate-200 bg-white shadow-sm p-5">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <span className="space-y-6">
          ...
          {errors.fileImage ? <p className="mt-0.5 text-xs text-rose-600">{errors.fileImage.message}</p> : null}
        </span>
        <span className="space-y-6">
          <div className="w-full h-fit aspect-video rounded-lg overflow-hidden relative">
            {validUrlPreview ? (
