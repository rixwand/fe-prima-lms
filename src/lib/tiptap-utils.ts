...
export const handleImageUpload = async (
  ...
  const path = `courses/${uuid()}.${ext}`;
  const { error, data } = await storageClient.from(SUPABASE_BUCKET).createSignedUploadUrl(path);
  if (error) {
  ...
  await uploadStreamed(file, data.signedUrl, onProgress);
  const urlImg = SUPABASE_URL + "/object/public/" + SUPABASE_BUCKET + "/" + data.path;
  return urlImg;
};
